...
export async function sendSignUpEmail(email: string, url: string, token: string) {
  ...
  const subject = 'Sign in to Split';
  const text = `Hey,\n\nYou can sign in to Split by clicking the below URL:\n${url}\n\nYou can also use this OTP: ${token}\n\nThanks,\nSplit Team`;
  const html = `<p>Hey,</p> <p>You can sign in to Split by clicking the below URL:</p><p><a href="${url}">Sign in to ${host}</a></p><p>You can also use this OTP: <b>${token}</b></p><br /><br /><p>Thanks,</p><br/>Split Team</p>`;
...
export async function sendInviteEmail(email: string, name: string) {
  ...
  const subject = 'Invitation to Split';
  const text = `Hey,\n\nYou have been invited to Split by ${name}. It's a completely open source free alternative to splitwise. You can sign in to Split by clicking the below URL:\n${env.NEXTAUTH_URL}\n\nThanks,\nSplit Team`;
  const html = `<p>Hey,</p> <p>You have been invited to Split by ${name}. It's a completely open source free alternative to splitwise. You can sign in to Split by clicking the below URL:</p><p><a href="${env.NEXTAUTH_URL}">Sign in to ${host}</a></p><br><p>Thanks,<br/>Split Team</p>`;
...
export async function sendFeedbackEmail(feedback: string, user: User) {
  ...
  const subject = `Feedback received on Split from ${user.name}`;
